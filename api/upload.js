...
async function handleSingleUpload(body, user, res) {
  ...
  // 分片上传模式：body.chunks 为分片数组，先拼接
  let fileBuffer;
  let fileName;
  if (body.chunks && Array.isArray(body.chunks) && body.chunks.length > 0) {
    fileBuffer = Buffer.concat(body.chunks.map(c => Buffer.from(c, 'base64')));
    fileName = body.fileName || 'upload.zip';
  } else {
    fileBuffer = Buffer.from(body.file || body.zip, 'base64');
    fileName = body.fileName || 'upload.zip';
  }
  ...
  // 发送邮件通知
  sendUploadEmail(user.userId, true, {
    repo: `${owner}/${repo}`,
    branch,
    fileName,
    fileCount: files.length,
