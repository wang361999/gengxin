...
async function verifyLicense(options = {}) {
  ...
  try {
    ...
    if (data.valid) {
      // ===== 域名绑定验证 =====
      // 远程验证通过后，检查域名是否与授权绑定的域名匹配
      try {
        const protection = require('./protection');
        // 如果授权信息中包含绑定域名，则验证当前域名是否匹配
        if (data.domain && !protection.verifyDomainBinding(domain, data.domain)) {
          isVerified = false;
          licenseInfo = null;
      ...
      try {
        const protection = require('./protection');
        if (data.signature) {
          const isValid = protection.verifyLicenseResultSignature(
            { valid: data.valid, domain: data.domain || domain },
            data.signature
          );
          if (!isValid) {
            isVerified = false;
            return { valid: false, code: 'signature_invalid', message: '授权结果签名验证失败' };
          }
        }
      } catch {
        // 签名验证异常时不阻止
 ...
 * @returns {Promise<Object>} 验证结果
 */
async function checkLicenseCached(options = {}) {
  // 内存缓存有效且在 24 小时内
  ...
  // 缓存过期，重新验证
  return await verifyLicense(options);
}
 ...
 * 未授权时返回 403
 *
 * 每次调用都从数据库读取最新配置，确保配置变更即时生效。
 * - license_enabled 不为 'true' 时直接放行
 * - license_key 为空时放行（视为未配置）
 * - 否则执行远程验证（带缓存，24 小时内不重复请求）
 *
 * 用法：
...
async function checkLicenseForRequest() {
  ...
  try {
    const protection = require('./protection');
    const protectionResult = protection.performProtectionCheck();
    if (!protectionResult.protected) {
      return { valid: false, code: 'protection_failed', message: '系统保护检查未通过' };
    }
  } catch {
    // 保护模块异常时不阻止（兼容性）
  }

  // 从数据库读取配置（每次请求都读，确保配置变更即时生效）
  let db;
  try {
    db = await getDb();
  } catch {
    // 数据库不可用时放行，避免锁死系统
    return { valid: true, code: 'db_error', message: '数据库不可用，跳过验证' };
  }

  let licenseEnabled, licenseKey, verifyUrl, licenseDomain;
  try {
    licenseEnabled = await db.getSetting('license_enabled');
    licenseKey = await db.getSetting('license_key');
    verifyUrl = await db.getSetting('license_verify_url') || 'https://gitd.cn/api/license/verify';
    licenseDomain = await db.getSetting('license_domain');
  } catch {
    return { valid: true, code: 'db_error', message: '读取配置失败，跳过验证' };
  }

  // 未启用授权验证，直接放行
  if (licenseEnabled !== 'true') {
    return { valid: true, code: 'disabled', message: '授权验证未启用' };
  }

  // 启用了但未配置授权码，拒绝访问
  if (!licenseKey) {
    return { valid: false, code: 'no_license', message: '已启用授权验证但未配置授权码，请在后台配置授权码' };
  }

  // 同步配置到内存（供 checkLicenseCached 使用）
  CONFIG.licenseKey = licenseKey;
  CONFIG.verifyUrl = verifyUrl;

  // 执行带缓存的验证
  const options = { licenseKey, verifyUrl };
  if (licenseDomain) options.domain = licenseDomain;

  return await checkLicenseCached(options);
}
