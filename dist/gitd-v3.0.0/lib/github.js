...
// 判断是否为「Token 写入权限不足」导致的 404
// 场景：GET 请求（读取文件树）成功，但 POST 请求（创建 tree/commit）返回 404
// GitHub 出于安全原因，对权限不足的写入操作返回 404 而非 403
function isWritePermissionError(status, method, prevReadSucceeded) {
  return status === 404 && method === 'POST' && prevReadSucceeded === true;
}

// 生成 Token 权限不足的友好错误提示
function permissionErrorMessage() {
  return 'Token 权限不足（仅有读取权限，缺少写入权限）。请前往 GitHub Settings → Developer settings → Personal access tokens，将 Token 的 Contents 权限改为 Read and write，然后更新仓库配置中的 Token。';
}

// 初始化空仓库：创建 README.md 并建立分支
async function initEmptyRepo(owner, repo, branch, token) {
  // 1. 创建 README.md blob
  const readmeContent = Buffer.from(`# ${repo}\n\nThis repository is managed by gitd.\n`).toString('base64');
...
async function uploadFiles(owner, repo, files, branch, commitMsg, token) {
  ...
  let readSucceeded = false; // 追踪读取操作是否成功
  ...
  if (ref.status === 404) {
    ...
    if (existingBranches.length === 0) {
      ...
      if (createRefRes.status < 200 || createRefRes.status >= 300) {
        if (isWritePermissionError(createRefRes.status, 'POST', readSucceeded)) {
          throw new Error(`创建分支 ${branch} 失败: ${permissionErrorMessage()}`);
        }
  ...
  for (const file of files) {
    ...
    if (r.status < 200 || r.status >= 300) {
      if (isWritePermissionError(r.status, 'POST', readSucceeded)) {
        throw new Error(`创建 blob 失败 (${safePath}): ${permissionErrorMessage()}`);
      }
      throw new Error(`创建 blob 失败 (${safePath}): ${explainError(r.status, r.data)}`);
    }
  ...
  for (let i = 0; i < blobs.length; i += 500) {
    ...
    if (r.status < 200 || r.status >= 300) {
      if (isWritePermissionError(r.status, 'POST', readSucceeded)) {
        throw new Error(`创建 tree 失败: ${permissionErrorMessage()}`);
      }
  ...
  if (commit.status < 200 || commit.status >= 300) {
    if (isWritePermissionError(commit.status, 'POST', readSucceeded)) {
      throw new Error(`创建 commit 失败: ${permissionErrorMessage()}`);
    }
...
// 清空仓库所有文件 - 完全清空，不保留任何文件
async function clearRepoFiles(owner, repo, branch, token) {
  // 追踪读取操作是否成功，用于区分「仓库不存在」与「Token写入权限不足」
  ...
  if (push.status < 200 || push.status >= 300) {
    if (push.status === 404 && readSucceeded) {
      throw new Error(`更新分支失败: ${permissionErrorMessage()}`);
    }
...
async function deleteFiles(owner, repo, branch, filePaths, token) {
  ...
  for (let i = 0; i < deleteItems.length; i += batchSize) {
    ...
    if (treeRes.status < 200 || treeRes.status >= 300) {
      // 如果 GET 读取成功但 POST 写入返回 404，说明 Token 权限不足（仅有读取权限）
      if (isWritePermissionError(treeRes.status, 'POST', readSucceeded)) {
        throw new Error(`创建 tree 失败: ${permissionErrorMessage()}`);
      }
      throw new Error(`创建 tree 失败: ${explainError(treeRes.status, treeRes.data)}`);
    }
...
async function createOrUpdateFile(owner, repo, path, content, branch, commitMsg, token) {
  ...
  if (checkRes.status === 200 && checkRes.data && checkRes.data.sha) {
    ...
    readSucceeded = true; // 404 也意味着仓库可访问，只是文件不存在
  }
  ...
  if (r.status < 200 || r.status >= 300) {
    // PUT 请求返回 404 且之前 GET 成功 = Token 权限不足
    if (r.status === 404 && readSucceeded) {
      throw new Error(`保存文件失败: ${permissionErrorMessage()}`);
    }
...
async function createFileInRepo(owner, repo, path, content, branch, commitMsg, token) {
  ...
  if (r.status < 200 || r.status >= 300) {
    if (r.status === 404 && readSucceeded) {
      throw new Error(`创建文件失败: ${permissionErrorMessage()}`);
    }
...
async function renameFile(owner, repo, oldPath, newPath, branch, commitMsg, token) {
  ...
  if (createRes.status < 200 || createRes.status >= 300) {
    if (createRes.status === 404 && readSucceeded) {
      throw new Error(`创建新文件失败: ${permissionErrorMessage()}`);
    }
  ...
  if (deleteRes.status < 200 || deleteRes.status >= 300) {
    if (deleteRes.status === 404 && readSucceeded) {
      throw new Error(`删除旧文件失败: ${permissionErrorMessage()}`);
    }
