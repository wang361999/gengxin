...
const { query, ensureDB, validateUserSession, validateAdminSession, getAppVersion, updateOnlineStatus, getSetting } = require('./db');

// ===== 授权保护模块（防篡改核心） =====
// 引入保护模块进行完整性校验
// 注意：此模块与 license-verifier 协同工作，请勿移除
let _protectionModule = null;
function _getProtection() {
  if (!_protectionModule) {
    try { _protectionModule = require('./protection'); } catch { _protectionModule = null; }
  }
  return _protectionModule;
}

// 授权验证：检查是否已启用并验证通过
// 管理员完全豁免（确保后台始终可用，可配置/修复授权）
async function checkLicense(req, isAdmin) {
  // ===== 第一层保护：代码完整性校验 =====
  // 检测关键模块是否被篡改
  const protection = _getProtection();
  if (protection) {
    const check = protection.performProtectionCheck();
    if (!check.protected && !isAdmin) {
      return { valid: false, message: check.message || '系统保护检查未通过' };
    }
  }
  ...
  try {
    ...
    return await checkLicenseForRequest();
  } catch {
    // SDK 异常时放行，避免锁死
    return { valid: true };
  }
...
async function requireAuth(req, res) {
  ...
  if (user.role === 'admin' && !user.userId) {
    ...
    // 更新在线状态（不阻塞主流程）
  updateOnlineStatus(user.adminId || 'admin_1', 'admin', user.username || 'admin').catch(() => {});
  // 授权验证（管理员豁免设置接口）
  const licenseResult = await checkLicense(req, true);
  if (!licenseResult.valid) {
    res.statusCode = 403;
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.end(JSON.stringify({ ok: false, error: '未授权', message: licenseResult.message || '系统未授权或授权已过期', code: 'unauthorized' }));
    return null;
  }
  ...
  // 更新在线状态（不阻塞主流程）
  updateOnlineStatus(user.userId, 'user', user.username || '').catch(() => {});
  // 授权验证
  const licenseResult = await checkLicense(req, false);
  if (!licenseResult.valid) {
    res.statusCode = 403;
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.end(JSON.stringify({ ok: false, error: '未授权', message: licenseResult.message || '系统未授权或授权已过期', code: 'unauthorized' }));
