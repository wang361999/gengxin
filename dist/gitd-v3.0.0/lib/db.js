  ...
  initPromise = (async () => {
    ...
    try {
      ...
      try {
        ...
        await client.query(`INSERT INTO settings (key, value) VALUES ('complaint_email', 'admin@gitd.cn') ON CONFLICT (key) DO NOTHING`);
        await client.query(`INSERT INTO settings (key, value) VALUES ('user_agreement', ${"'" + `gitd 用户协议

最后更新日期：2026年7月30日

欢迎使用 gitd（以下简称"本平台"或"本工具"）。本平台是一项基于 GitHub API 的代码仓库管理工具，提供文件上传、仓库管理、批量操作等功能。请仔细阅读以下协议条款。使用本平台即表示你同意本协议的全部内容。
...
本协议的解释和适用适用中华人民共和国法律。如有争议，双方应友好协商解决；协商不成的，提交有管辖权的人民法院裁决。` + "'"}) ON CONFLICT (key) DO NOTHING`);
        await client.query(`INSERT INTO settings (key, value) VALUES ('privacy_policy', ${"'" + `gitd 隐私政策

最后更新日期：2026年7月30日

本隐私政策说明 gitd（以下简称"本平台"）如何收集、使用和保护你的个人信息。使用本平台即表示你同意本隐私政策。
...
十、联系我们
如对本隐私政策有任何疑问，请通过邮箱联系：admin@gitd.cn` + "'"}) ON CONFLICT (key) DO NOTHING`);
      } catch {}
