...
// 授权验证：检查是否已启用并验证通过
// 豁免管理员访问设置相关接口（否则授权失败后无法修复配置）
async function checkLicense(req, isAdmin) {
  // 管理员访问 settings / license-test 接口时豁免
  if (isAdmin) {
    const url = (req.url || '').toLowerCase();
    if (url.includes('settings') || url.includes('license-test') || url.includes('version') || url.includes('health')) {
      return { valid: true };
    }
  }
