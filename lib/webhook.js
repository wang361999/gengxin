...
function formatFeishuMessage(event, data) {
  ...
  return {
    ...
    card: {
      header: { template: color, title: { tag: 'plain_text', content: 'GitShip 上传通知' } },
...
function formatDingtalkMessage(event, data) {
  ...
  return {
    msgtype: 'markdown',
    markdown: { title: 'GitShip 通知', text }
  };
}
...
function formatDiscordMessage(event, data) {
  ...
  return {
    embeds: [{
      title: `GitShip 通知 - ${event}`,
      color,
      fields,
      footer: { text: 'GitShip SaaS' },
