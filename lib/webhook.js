...
function formatFeishuMessage(event, data) {
  ...
  return {
    ...
    card: {
      header: { template: color, title: { tag: 'plain_text', content: 'GitUpload 上传通知' } },
...
function formatDingtalkMessage(event, data) {
  ...
  return {
    msgtype: 'markdown',
    markdown: { title: 'GitUpload 通知', text }
  };
}
...
function formatDiscordMessage(event, data) {
  ...
  return {
    embeds: [{
      title: `GitUpload 通知 - ${event}`,
      color,
      fields,
      footer: { text: 'GitUpload SaaS' },
